...
def kNN_model(X_train, X_test, y_train, y_test):
    ...
    print(f"Estimator: K-nearest neighbors:\n"
          f"Optimal k={k_best_u + k_range[0]} with an accuracy of {scores_u[k_best_u]} with uniform weight method.\n"
          f"Optimal k={k_best_d + k_range[0]} with an accuracy of {scores_d[k_best_d]} with distance weight method.")
...
def group_model(X_test, X_test_scaled, y_test, models, *, weights=None):
    ...
    ** Creating a group/ensemble model of all of the fitted models ** 
    ...
    n = len(models)
    wgts = np.ones(n) / n if weights == None else weights
